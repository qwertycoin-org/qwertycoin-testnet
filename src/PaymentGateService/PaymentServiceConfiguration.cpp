...
void Configuration::initOptions(po::options_description& desc) {
    desc.add_options()
            ...
            ("bind-port", po::value<uint16_t>()->default_value((uint16_t) CryptoNote::SERVICE_DEFAULT_PORT), "payment service bind port")
            ("bind-port-ssl", po::value<uint16_t>()->default_value((uint16_t) CryptoNote::SERVICE_DEFAULT_SSL_PORT), "payment service bind port ssl")
