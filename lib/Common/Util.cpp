    ...
    if (!boost::filesystem::exists(config_folder) && boost::filesystem::is_directory(old_path)) {
        if (boost::filesystem::create_directory(config_folder)) {
            for (const auto &entry : boost::filesystem::recursive_directory_iterator { old_path }) {
                ...
                boost::filesystem::copy(path,
                                        config_folder + boost::filesystem::path::preferred_separator
                                                + rel_path_str);
            }
...
std::error_code replace_file(const std::string &replacement_name, const std::string &replaced_name)
{
    ...
    bool ok = 0
            != ::MoveFileEx(replacement_name.c_str(), replaced_name.c_str(),
...
bool parseUrlAddress(const std::string &url, std::string &host, uint16_t &port, std::string &path,
                     bool &ssl)
{
    bool res = true;
    ...
    if (boost::regex_match(url.c_str(), reg_res, uri_exp)) {
        if (reg_res.length(4) > 0) {
            ...
            if (sscanf(reg_res.str(4).c_str() + 1, "%d", &port_src) == 1) {
                if (port_src > 0 && port_src <= 0xFFFF)
                    port = (uint16_t)port_src;
            }
        } else {
            if (strcmp(reg_res.str(1).c_str(), "http://") == 0)
                port = HTTP_PORT;
            else if (strcmp(reg_res.str(1).c_str(), "https://") == 0)
                port = HTTPS_PORT;
            else
                port = CryptoNote::RPC_DEFAULT_PORT;
        }
        if (port != 0) {
            if (strcmp(reg_res.str(1).c_str(), "https://") == 0)
                ssl = true;
