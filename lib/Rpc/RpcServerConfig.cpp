...
const command_line::arg_descriptor<uint16_t> arg_rpc_bind_port = { "rpc-bind-port", "",
                                                                   DEFAULT_RPC_PORT };
const command_line::arg_descriptor<bool> arg_rpc_bind_ssl_enable = { "rpc-bind-ssl-enable",
                                                                     "Enable SSL for RPC service",
                                                                     false };
const command_line::arg_descriptor<uint16_t> arg_rpc_bind_ssl_port = { "rpc-bind-ssl-port",
                                                                       "SSL port for RPC service",
                                                                       DEFAULT_RPC_SSL_PORT };
const command_line::arg_descriptor<std::string> arg_chain_file = { "rpc-chain-file",
                                                                   "SSL chain file",
                                                                   DEFAULT_RPC_CHAIN_FILE };
const command_line::arg_descriptor<std::string> arg_key_file = { "rpc-key-file", "SSL key file",
                                                                 DEFAULT_RPC_KEY_FILE };
const command_line::arg_descriptor<std::string> arg_dh_file = { "rpc-dh-file", "SSL DH file",
                                                                DEFAULT_RPC_DH_FILE };
const command_line::arg_descriptor<bool> arg_restricted_rpc = {
    "restricted-rpc", "Restrict RPC to view only commands to prevent abuse", false
};
const command_line::arg_descriptor<std::string> arg_enable_cors = {
    "enable-cors",
    "Adds header 'Access-Control-Allow-Origin' to the daemon's RPC responses. Uses the value as "
    "domain. Use * for all",
    ""
};
const command_line::arg_descriptor<std::string> arg_set_contact = { "contact",
                                                                    "Sets node admin contact", "" };
const command_line::arg_descriptor<std::string> arg_set_fee_address = {
    "fee-address", "Sets fee address for light wallets.", ""
};
const command_line::arg_descriptor<std::string> arg_set_fee_amount = {
    "fee-amount", "Sets flat rate fee for light wallets.", ""
};
const command_line::arg_descriptor<std::string> arg_set_view_key = {
    "view-key", "Sets private view key to check for node's fee.", ""
};
...
bool RpcServerConfig::isEnabledSSL() const
{
    return enableSSL;
}

uint16_t RpcServerConfig::getBindPort() const
{
    return bindPort;
}

uint16_t RpcServerConfig::getBindPortSSL() const
{
    return bindPortSSL;
}

std::string RpcServerConfig::getBindIP() const
{
    return bindIp;
}

std::string RpcServerConfig::getDhFile() const
{
    return dhFile;
}

std::string RpcServerConfig::getChainFile() const
{
    return chainFile;
}

std::string RpcServerConfig::getKeyFile() const
{
    return keyFile;
}
...
std::string RpcServerConfig::getBindAddressSSL() const
{
    return bindIp + ":" + std::to_string(bindPortSSL);
}
