    ...
    while (true) {
        ...
        if (cert_end != std::string::npos) {
            ctx.add_certificate_authority(
                    boost::asio::buffer(emb_certs.data() + cert_start, cert_end - cert_start));
            cert_start = cert_end;
        } else {
            break;
        }
...
void sockSetup(SOCKET &sock)
{
    ...
    setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, (const char *)&enable_keep_alive,
               sizeof(enable_keep_alive));
...
{
    if (this->m_sslCert.empty())
        this->m_sslCert = path;
}

void HttpClient::disableVerify()
{
    if (!this->m_sslNoVerify)
        this->m_sslNoVerify = true;
}
    ...
    if (this->m_sslEnable) {
        try {
            ...
            while (write_full_size < req_data_size) {
                write_size = this->m_sslSock->write_some(boost::asio::buffer(
                        req_data.data() + write_full_size, req_data_size - write_full_size));
                if (write_size > 0) {
            ...
            while (true) {
                ...
                resp_size = this->m_sslSock->read_some(
                        boost::asio::buffer((char *)resp_buff, resp_buff_size));
                ...
                if (resp_size > 0) {
                    ...
                    if (!header_found) {
                        ...
                        if (header_end != std::string::npos) {
                            ...
                            size_t content_lenght_end =
                                    data.find(content_lenght_end_sep, content_lenght_start);
                            if (content_lenght_start != std::string::npos
                                && content_lenght_end != std::string::npos) {
                                sscanf(data.substr(content_lenght_start
                                                           + strlen(content_lenght_name) + 2,
                                                   content_lenght_end - content_lenght_start
                                                           - strlen(content_lenght_name) - 2)
                                               .c_str(),
                                       "%zu", &stream_len);
                    ...
                    if (header_found) {
                        if (stream_len > 0) {
                            if (resp_size_full >= stream_len)
                                break;
                        } else {
                            if (resp_size_full == header_end + 4)
                                break;
                        }
                    }
                } else {
    ...
    if (this->m_sslEnable) {
        try {
            ...
            if (this->m_sslCert.empty()) {
                ...
                ctx.load_verify_file(m_sslCert);
            }
            this->m_sslSock.reset(
                    new boost::asio::ssl::stream<tcp::socket>(this->m_ioService, std::ref(ctx)));
            ...
            if (!this->m_sslNoVerify) {
                this->m_sslSock->set_verify_mode(boost::asio::ssl::verify_peer);
            } else {
                this->m_sslSock->set_verify_mode(boost::asio::ssl::verify_none);
            }
            this->m_sslSock->set_verify_callback(boost::asio::ssl::rfc2818_verification(hostname));
            this->m_sslSock->handshake(boost::asio::ssl::stream_base::client);
            m_connected = true;
...
void HttpClient::disconnect()
{
    if (this->m_sslEnable) {
        try {
            this->m_sslSock->lowest_layer().close();
        } catch (std::exception&) {
