...
void HttpServer::setCerts(const std::string &chain_file, const std::string &key_file,
...
void HttpServer::start(const std::string &address, uint16_t port, uint16_t portSSL,
                       bool serverSSLEnable, const std::string &user, const std::string &password)
{
    m_listener = System::TcpListener(m_dispatcher, System::Ipv4Address(address), port);
    workingContextGroup.spawn(std::bind(&HttpServer::acceptLoop, this));

    this->m_server_ssl_do = serverSSLEnable;
    this->m_server_ssl_port = portSSL;
...
void HttpServer::sslServerUnitControl(boost::asio::ssl::stream<tcp::socket &> &stream,
                                      boost::system::error_code &ec, bool &unit_do,
                                      bool &unit_control_do, size_t &stream_timeout_n)
{
...
void HttpServer::sslServerUnit(boost::asio::ip::tcp::socket &socket, boost::asio::ssl::context &ctx)
{
    ...
    boost::thread control_t(std::bind(&HttpServer::sslServerUnitControl, this, std::ref(stream),
                                      std::ref(ec), std::ref(unit_do), std::ref(unit_control_do),
        ...
        if (!ec) {
            ...
            while (keep_alive_conn) {
                ...
                while (unit_do) {
                    size_t req_size = 0;
                    if (unit_do)
                        req_size = stream.read_some(
                                boost::asio::buffer((char *)req_buff + req_size_full,
                    ...
                    if (header_found) {
                        if (stream_len > 0) {
                            if (req_size_full >= stream_len)
                                break;
                        } else {
                            if (req_size_full == header_end + 4)
                                break;
                        }
                    }
                }
                if (req_size_full > 0 && req_size_full < request_max_len && unit_do) {
                    ...
                    while (resp_size_full < resp_size_data && unit_do) {
                        size_t resp_size = 0;
                        if (unit_do)
                            resp_size = stream.write_some(
                                    boost::asio::buffer(resp_data.data() + resp_size_full,
...
void HttpServer::sslServer()
{
    while (this->m_server_ssl_do) {
        ...
        try {
            ...
            tcp::acceptor accept(
                    io_service,
                    tcp::endpoint(boost::asio::ip::address::from_string(this->m_address),
                                  this->m_server_ssl_port));

            boost::thread control_t(
                    std::bind(&HttpServer::sslServerControl, this, std::ref(accept)));
            control_t.detach();
            ...
            while (this->m_server_ssl_do) {
                ...
                if (this->m_server_ssl_do) {
                    boost::thread t(std::bind(&HttpServer::sslServerUnit, this, std::move(sock),
                                              std::ref(ctx)));
                    t.detach();
                }
            }
...
void HttpServer::acceptLoop()
{
    try {
        ...
        auto addr =
                std::pair<System::Ipv4Address, uint16_t>(static_cast<System::Ipv4Address>(0), 0);
        try {
